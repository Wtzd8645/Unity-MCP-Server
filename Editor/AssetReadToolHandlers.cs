...
namespace Blanketmen.UnityMcp.Control.Editor
{
    ...
    {
        public static ControlToolCallResponse HandleAssetSearch(ControlToolCallRequest request)
        {
            AssetSearchArgs args = ControlJson.ParseArgs(
            ...
            string[] folders = ControlUtil.BuildSearchFolders(args.pathPrefixes, args.includePackages);
            ...
            PaginationRange range = ControlUtil.BuildPaginationRange(total, args.offset, args.limit, 500);
        ...
        {
            AssetGetArgs args = ControlJson.ParseArgs(
                request.argumentsJson,
                new AssetGetArgs
                {
            ...
            return ControlResponses.Success("unity_asset_get completed.", payload);
        }

        public static ControlToolCallResponse HandleAssetRefs(ControlToolCallRequest request)
        {
            AssetRefsArgs args = ControlJson.ParseArgs(
                request.argumentsJson,
                new AssetRefsArgs
                {
                    direction = "inbound",
        ...
        private static bool TryResolveAssetRef(AssetRef target, out string path, out string guid)
        {
            ...
            {
                if (!ControlWriteSupport.TryNormalizeAssetPath(target.path, out string normalizedPath, out _))
                {
                    return false;
                }
            ...
            if (!ControlWriteSupport.TryValidateAssetPathAllowed(path, out _))
            {
                return false;
            }
