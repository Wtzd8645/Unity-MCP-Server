...
namespace Blanketmen.UnityMcpBridge.Editor
{
    ...
    {
        public static BridgeToolCallResponse HandleGetConsoleLogs(BridgeToolCallRequest request, UnityBridgeLogStore logStore)
        {
            GetConsoleLogsArgs args = BridgeJson.ParseArgs(
            ...
            long sinceId = BridgeUtil.ParseLong(args.sinceId, 0);
            HashSet<string> levels = BuildLevelSet(args.levels);
            LogSnapshot snapshot = logStore.Snapshot();

            IEnumerable<BridgeLogEntry> filtered = snapshot.entries
            ...
            int limit = BridgeUtil.Clamp(args.limit, 1, 2000, 200);
            List<BridgeLogEntry> page = filtered.Take(limit).ToList();

            var items = new List<ConsoleLogItem>(page.Count);
            long nextSinceId = snapshot.maxId;
            for (int i = 0; i < page.Count; i++)
            {
                BridgeLogEntry log = page[i];
        ...
        {
            RunTestsArgs args = BridgeJson.ParseArgs(
                request.argumentsJson,
                new RunTestsArgs
                {
                    mode = "EditMode",
